    ...
    def create
      ...
      unless @conversation.participants.include?(current_user)
        respond_to do |format|
          format.html { redirect_to seller_conversations_path, alert: "Vous n'avez pas accès à cette conversation." }
      ...
      respond_to do |format|
        if @message.save
          ...
          format.html { redirect_to seller_conversations_path(conversation_id: @conversation.id), notice: "Message envoyé." }
          format.turbo_stream
        else
          format.html { redirect_to seller_conversations_path(conversation_id: @conversation.id), alert: "Impossible d'envoyer le message." }
