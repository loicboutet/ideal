  ...
  def create
    ...
    unless @conversation.participants.include?(current_user)
      respond_to do |format|
        format.html { redirect_to conversations_path, alert: "Vous n'avez pas accès à cette conversation." }
    ...
    respond_to do |format|
      if @message.save
        ...
        format.html { redirect_to @conversation, notice: "Message envoyé." }
        format.turbo_stream
      else
        format.html { redirect_to @conversation, alert: "Impossible d'envoyer le message." }
